...
awful.keyboard.append_global_keybindings({
      ...
      -- Mover ventana al monitor siguiente
    awful.key({ modkey, "alt" }, "h", function()
        if client.focus then
            local c = client.focus
            c:move_to_screen(c.screen.index + 1)
        end
    end, { description = "Move window to the next screen", group = "Screen" }),

    -- Mover ventana al monitor anterior
    awful.key({ modkey, "alt" }, "l", function()
        if client.focus then
            local c = client.focus
            c:move_to_screen(c.screen.index - 1)
        end
    end, { description = "Move window to the previous screen", group = "Screen" }),
    ...
    awful.key({ modkey, shift }, "s", function()
		awful.spawn.easy_async({ "rofi-screenshot"})
...
client.connect_signal("request::default_keybindings", function()
	awful.keyboard.append_client_keybindings({
		...
		awful.key({ modkey }, "w", function(c)
          if not string.find(c.name, "Minecraft") or c.name == "Minecraft Launcher" then
              c:kill()
          end
		...
		awful.key({ modkey, ctrl }, ".", function(c)
